...
window.App = {
  start: function() {
    ...
    // Bootstrap the Wallet abstraction for Use.
  ...
  listenToEvents: function() {
    MyWallet.deployed().then(function(instance) {
      instance.receivedFunds({},{fromBlock: 0, toBlock: 'latest'}).watch(function(error, event) {
        ...
        var amount = web3.fromWei(event.args._amount, "ether");
        document.getElementById("fundEvents").innerHTML += "from: " + from + " amount: " + amount + "<br />";
      });
      instance.proposalReceived({},{fromBlock: 0, toBlock: 'latest'}).watch(function(error, event) {
        var from = event.args._from;
        var to = event.args._to;
        var reason = event.args._reason;
        var proposal_id = event.args.proposal_id;

        document.getElementById("proposalEvents").innerHTML += "from: " + from + " to: " + to + " reason: " + reason + " proposal_id: " + proposal_id + "<br />";
      });
    });
  },
  ...
  confirmTransaction: function() {
    MyWallet.deployed().then(function(instance) {
      ...
      return instance.confirmProposal(proposal_id, {owner: accounts[0], from: accounts[0]});
    })
