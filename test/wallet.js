...
contract("MyWallet", function(accounts) {
    ...
    it('Propose to spending money', function() {
        var walletInstance;
        return wallet.deployed().then(function(instance) {
            walletInstance = instance;
            return walletInstance.confirmProposal(1, {owner: accounts[0], from: accounts[0]});
        }).then(function() {
            assert.equal(web3.eth.getBalance(walletInstance.address).toNumber(), web3.toWei(0, 'ether'), 'Balance is now 0 ether');
        });
    });
